...
object CacheUtil {
    ...
    fun getTotalCacheSize(): String? {
        var cacheSize = getFolderSize(BaseApp.getInstance().cacheDir)
        if (Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED) {
            cacheSize += BaseApp.getInstance().externalCacheDir?.let { getFolderSize(it) }!!
        }
     ...
     */
    fun clearAllCache() {
        deleteDir(BaseApp.getInstance().cacheDir)
        if (Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED) {
            deleteDir(BaseApp.getInstance().externalCacheDir)
        }
    }
